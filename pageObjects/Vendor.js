...
class Vendor {
  constructor(page) {
    ...
    this.emailExistMsg = page.getByText("Email ID already exists.");
  }

  getVendorListRows() {
    return this.page
      .getByRole("button")
      .filter({ hasText: VENDOR_ROW_PATTERN });
  }
  ...
  storeVendorListCounts(listBody) {
    sessionDataStorage.set("vendor_APIcount", Number(listBody.total_vendors));
    if (listBody.total_assigned_products != null) {
      sessionDataStorage.set(
        "vendor_productsSupplied_APIcount",
        Number(listBody.total_assigned_products),
      );
    }
    if (listBody.total_vendors_with_no_product != null) {
      sessionDataStorage.set(
        "vendor_noProducts_APIcount",
        Number(listBody.total_vendors_with_no_product),
      );
    }
  }

  async clearVendorSearch() {
    const currentValue = (await this.searchBar.inputValue())?.trim() ?? "";
    if (!currentValue) {
      return;
    }
    ...
    if (listResponse) {
      this.storeVendorListCounts(await listResponse.json());
    }
  }
  ...
  async DuplicateErrorDisplay() {
    return await this.emailExistMsg.isVisible();
  }

  async verifyDuplicateEmailErrorVisible() {
    await expect(this.emailExistMsg).toBeVisible({ timeout: 10_000 });
  }

  async LegitEmail(vendorName, phone = "5551234567", email) {
    await this.addVendorBtnClick();
    await this.fillVendorForm({ vendorName, phone, email });
  }

  async verifyDuplicateEmailNotAllowed(
    existingVendorName,
    existingPhone = "5551234567",
    email = `dup.${Date.now()}@example.com`,
  ) {
    // 1) Create a vendor so this email is already in use
    await this.createVendor({
      vendorName: existingVendorName,
      phone: existingPhone,
      email,
    });
    await this.verifyVendorListedAtTop(existingVendorName);

    // 2) Try another vendor with the same email → validation error
    // UI shows "Email ID already exists." but keeps Add enabled.
    const otherVendorName = this.generateUniqueVendorName();
    await this.LegitEmail(otherVendorName, "5559998888", email);
    await this.verifyDuplicateEmailErrorVisible();
    await this.verifyVendorNotAdded();
    await this.cancelAddVendorModal();

    // 3) Cleanup so the suite can re-run
    await this.clickDeleteVendor(existingVendorName);
    await this.deleteVendorAPI();
    await this.verifyDeletedVendorNotInSearch(existingVendorName);
  }

  async searchVendor(searchText) {
    const searchTerm = this.getVendorSearchTerm(searchText);
    // Do not store list totals here — filtered search totals are not global counts.
    await Promise.all([
      this.page
        .waitForResponse(
          (res) =>
  ...
  async refreshVendorsPage() {
    ...
    this.storeVendorListCounts(body);
  ...
  async addVendorModalBtnDisable() {
  ...
  async saveEditVendorAPI() {
    ...
    // Edit opens via search; clear so stored totals are global, not filtered.
    await this.clearVendorSearch();
  }
  ...
  async deleteVendorAPI() {
    ...
    // findVendorRow leaves search filtered; that list's total_vendors is often 0
    // after delete. Clear search and sync global totals before asserting count.
    await this.clearVendorSearch();

    const newApiCount = sessionDataStorage.get("vendor_APIcount");
    expect(
      newApiCount,
  ...
  async verifyDeleteSuccessMessage(vendorName) {
    ...
    // Sync global totals after clearing filtered search from findVendorRow.
    await this.clearVendorSearch();
  }
}
