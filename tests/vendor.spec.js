...
test.describe("Vendors Module", () => {
  ...
  test("Duplicate vendor email shows error", async () => {
    const vendorName = vendor.generateUniqueVendorName();
    const email = vendor.generateUniqueVendorEmail();
    await vendor.verifyDuplicateEmailNotAllowed(vendorName, "5551234567", email);
  });
